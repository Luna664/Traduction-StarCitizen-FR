...
function Show-Menu {
    ...
    Write-Host
    Write-Host "Traduction de Circuspes:"
    Write-Host "5. Traduire la version Live"
    Write-Host "6. Traduire la version PTU"
    Write-Host "7. Recuperer le fichier de traductions depuis Internet"
    Write-Host "8. Participer a la traduction"
    Write-Host
    Write-Host "Menu:"
    ...
    switch ($choice) {
        '1' {
            ...
            if (-not (Test-Path -Path $localization1 -PathType Container)) {
                Write-Host "Je ne trouve pas le Live. Veuillez vérifier que vous m'avez placé dans le répertoire StarCitizen et que la version Live est installé."
        ...
        '2' {
            ...
            if (-not (Test-Path -Path $localization2 -PathType Container)) {
                Write-Host "Je ne trouve pas la version PTU. Veuillez vérifier que vous m'avez placé dans le répertoire StarCitizen et que la version PTU est installé."
                Read-Host "Appuyez sur Entrée pour continuer..."
            }
            else {
        ...
        '7' {
            ...
            # Téléchargement du fichier de traduction depuis Internet
            Write-Host "Téléchargement du fichier de traduction depuis Internet"
            Invoke-WebRequest -Uri "https://traduction.circuspes.fr/download/global.ini" -OutFile "$ScriptDir\global.ini"
